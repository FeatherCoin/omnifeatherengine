...
def getSource(sp):
  try:
    convert={0:{"cmcid":"1","name":"FTC","source":"coinmarketcap"}
            }
    if sp == 'cmcids':
      q=[]
...
def getfixedprice(desiredvalue, base):
  ...
  ROWS=dbSelect("select rate1for2 from exchangerates where protocol1='Fiat' and propertyid1=%s and protocol2='Feathercoin' and propertyid2=0 "
...
def updateOMNISP():
  try:
    ...
    for x in ROWS:
      ...
      upsertRate('Feathercoin', 0, 'Omni', sp, value, source)
